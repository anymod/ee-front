...
module = angular.module 'ee-layers', []

module.directive "eeLayers", ($state, $window, $timeout, eeCollections) ->
  templateUrl: 'components/ee-layers.html'
  restrict: 'E'
  scope:
    layers: '='
    canvasWidth: '@'
  ...
  link: (scope, ele, attrs) ->
    ...
    scope.canvasHeight  ||= 470
    scope.layers        ||= []

    padding = 0
    scope.base_path = 'https://res.cloudinary.com/eeosk/image/upload'
    scope.base_transform = 'w_' + scope.canvasWidth + ',h_' + scope.canvasHeight
    ...
    validateLayers = () ->
      for layer in scope.layers
        if !layer.base
          if layer.w > scope.canvasWidth  then layer.w = scope.canvasWidth - layer.x
          if layer.h > scope.canvasHeight then layer.h = scope.canvasHeight - layer.y
